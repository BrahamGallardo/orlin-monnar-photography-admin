...
const filters = computed((): AppointmentFilters => {
  const rawPage = Number.parseInt(
    readSingle(route.query[QUERY_KEYS.page]) ?? "",
    10,
  );
...
const fetchPage = (
  ...
  getAppointments({
    ...
    startDate: hasUsableRange.value
      ? startOfLocalDay(current.from as string)
      : null,
...
/** Appointment whose detail is open, as encoded in the query string. */
const detailId = computed((): number | null => {
  const raw = Number.parseInt(
    readSingle(route.query[QUERY_KEYS.detail]) ?? "",
    10,
  );

  return Number.isNaN(raw) || raw < 1 ? null : raw;
});

/** Closes the detail, keeping the filters untouched. */
const closeDetail = (): void => {
  const query = { ...route.query };
  delete query[QUERY_KEYS.detail];

  void router.push({ name: "Appointments", query });
};

/**
 * Reflects a status change in the table.
 *
 * @param updated - Appointment as the server left it.
 * @remarks
 * The row is replaced first so the change shows immediately, and the page is read
 * again because a status filter may no longer match the appointment: `filtersKey` does
 * not change here, so the watch would not fire on its own.
 */
const onAppointmentUpdated = (updated: AppointmentDto): void => {
 ...
 */
const applyFilters = (
  changes: Partial<Record<keyof typeof QUERY_KEYS, string | null>>,
): void => {
  const next: Record<string, string> = {};
  const merged = {
    status:
      changes.status !== undefined ? changes.status : filters.value.status,
    ...
    page:
      changes.page !== undefined
        ? changes.page
        : String(filters.value.pageIndex),
  };
 ...
 * @remarks
 * The identifier travels in the query string instead of a nested route: the detail is
 * a dialog over this view, and keeping it there makes the link shareable and survives
 * a reload without duplicating the filters in a second route.
 */
  ...
  <div class="space-y-6">
    <div>
      ...
      <p class="text-muted-foreground">
        Consulta y filtra las citas agendadas.
      </p>
    </div>
